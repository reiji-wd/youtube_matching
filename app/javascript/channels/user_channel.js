  ...
  consumer.subscriptions.create("UserChannel", {
    ...
    received(data) {
        ...
        if(data.notice) {
          ...
          const newNoticeCount = document.querySelector('.new-notice-count');
        ...
        if(data.message) {
          ...
          const roomId = document.querySelector(`.room-${data.message.room_id}`)
          const roomShow = document.querySelector(`[data-room-id="${data.message.room_id}"]`);
          const newMessageCount = document.querySelector(`.new-message-count-${data.message_room_id}`)
          // ルーム一覧ページにいれば実行
          if(rooms != undefined) {
            ...
            if(newMessageCount == undefined) {
              ...
              const counts = Number(newMessageCount.textContent);
              counts += 1;
              String(counts)
